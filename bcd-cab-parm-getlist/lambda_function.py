...
def lambda_handler(event, context):
    ...
    msg = {}
    records = []
    response = execute_statement('SELECT parmname,parmtype,parmval_dt,parmval_st,parmval_nu,last_updated FROM parm')
    for record in response['records']:
        ...
        if parm['parmtype'] == 'number': parm['parmval'] = record[4]['doubleValue']

...
def lambda_handler(event, context):
    ...
    sql="UPDATE sec_user SET email = :email, first_nme = :first_nme, last_nme = :last_nme, pw = :pw, last_updated = CURRENT_TIMESTAMP WHERE user_id = :user_id"
    parameter = [{'name':'email','value':{'stringValue':event['email']}},{'name':'first_nme','value':{'stringValue':event['first_nme']}}, {'name':'last_nme','value':{'stringValue':event['last_nme']}}, {'name':'pw','value':{'stringValue':passwordHash}},{'name':'user_id','value':{'longValue':event['user_id']}}]
